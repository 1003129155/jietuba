...
- **コピー元を問わず自動で履歴に**：自分で撮ったスクリーンショットだけでなく、コピーしたテキスト・画像・ファイルはすべて同じクリップボード履歴に自動で入ります

- **履歴 → ピン留め → 履歴**：履歴内のどの画像もワンクリックで画面最前面にピン留めでき、いつでも見比べたり、拡大縮小したり、続けて注釈を付けられます

- **数秒で見つかる検索**：キーワード全文検索に加え、時間・種類でのフィルター、独自のグループ分け・お気に入りにも対応。履歴が数百〜数千件になっても数秒で見つかります

- **メモリ使用量**：メモリ設計を徹底しており、バックグラウンド常駐時のメモリ使用量を数MB〜十数MBレベルまで抑えられます

- **快適なUI**：負荷の高いシーンも最適化済みで、低スペックPCでも快適に動作します

- **データの安全性**：すべての機能が端末上だけで完結し、データ収集も裏でのネット通信もありません。データは常にローカルに保存されます。唯一ネット接続が必要なのは翻訳機能で、これもユーザーが自分で選んだ翻訳APIを能動的に使ったときだけ通信します
