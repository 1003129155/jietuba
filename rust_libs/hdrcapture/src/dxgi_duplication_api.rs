...
use crate::d3d11::{MappedStagingTexture, StagingTexture, create_d3d_device_for_monitor, unmap_staging_texture};
...
impl DxgiDuplicationApi {
    ...
    pub fn new(monitor: Monitor) -> Result<Self, Error> {
        ...
        let (d3d_device, d3d_device_context) =
            create_d3d_device_for_monitor(windows::Win32::Graphics::Gdi::HMONITOR(monitor.as_raw_hmonitor()))?;

        Self::new_with_device_and_formats(monitor, d3d_device, d3d_device_context, &DEFAULT_DUPLICATION_FORMATS)
    }
    ...
    pub fn new_options(monitor: Monitor, supported_formats: &[DxgiDuplicationFormat]) -> Result<Self, Error> {
        ...
        Self::new_with_device_and_formats(monitor, d3d_device, d3d_device_context, &supported_formats)
    }
