...
def test_the_opacity_label_never_resizes_as_the_number_grows(qapp):
    """99%→100% 时整条控件不能抖一下：宽度固定，且宽到放得下每一个值。"""
    from ui.text_settings_panel import TextSettingsPanel

    panel = TextSettingsPanel()
    label = panel.background_opacity_label
    fixed = label.maximumWidth()
    assert label.minimumWidth() == fixed, "宽度没固定住，数字变长就会推着旁边的控件动"

    metrics = label.fontMetrics()
    for alpha in range(0, 256):
        panel.background_opacity_slider.setValue(alpha)
        assert (label.minimumWidth(), label.maximumWidth()) == (fixed, fixed)
        assert metrics.horizontalAdvance(label.text()) <= fixed, f"{alpha} 放不下"


def test_text_background_opacity_shows_a_percentage(qapp):
    """同一个面板里透明度不能有两种刻度：别处是百分比，滑块也得是。"""
    from ui.text_settings_panel import TextSettingsPanel

    panel = TextSettingsPanel()
    panel.background_opacity_slider.setValue(255)
    assert panel.background_opacity_label.text() == "100%"

    panel.background_opacity_slider.setValue(0)
    assert panel.background_opacity_label.text() == "0%"

    from PySide6.QtGui import QColor
    panel.set_background_settings(True, QColor("#FFFFFF"), 128)
    assert panel.background_opacity_label.text() == "50%"
