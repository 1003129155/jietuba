...
# ============================================================================
# accept() 必须保存每一家已注册服务商的凭据
# ============================================================================

def test_accept_persists_every_registered_provider_credential():
    """accept() 里那串保存是手写的，漏一家不会报错，只会静默丢数据。

    漏掉时的表现：设置页填了凭据、点保存、下次打开是空的，翻译一直说"未配置"。
    hasattr 守卫把 AttributeError 也吞了，所以连崩溃都不会有。百度就这么漏过
    一次——五家里只有它没被保存，而当时全部测试都是绿的。

    这里用 AST 取出 accept() 里所有 self.config_manager.set_xxx(...) 的方法名，
    再要求每个 provider 的 CREDENTIAL_FIELDS.config_key 都有对应的 setter。
    比调用 accept() 可靠：那个方法有两百多行、牵扯十几个子页面控件，
    用假 self 驱动的成本远高于它能挡住的问题。

    真正的解法是让设置页像欢迎页那样从 CREDENTIAL_FIELDS 反射生成读写，
    那之后这个测试就没有存在意义了，可以一并删掉。
    """
    import ast
    import inspect
    import textwrap
    ...
    tree = ast.parse(textwrap.dedent(inspect.getsource(SettingsDialog.accept)))
    saved = set()
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        func = node.func
        # 匹配 self.config_manager.set_xxx(...)
        if (
            isinstance(func, ast.Attribute)
            and isinstance(func.value, ast.Attribute)
            and func.value.attr == "config_manager"
            and isinstance(func.value.value, ast.Name)
            and func.value.value.id == "self"
        ):
            saved.add(func.attr)

    registry = create_default_translation_service(_Config()).registry
    missing = []
    for metadata in registry.available_providers():
        for field in metadata.credentials:
            setter = "set_" + field.config_key
            if setter not in saved:
                missing.append(
                    "%s -> accept() 没有调用 %s" % (metadata.provider_id, setter)
                )

    joined = chr(10).join('  ' + item for item in missing)
    assert not missing, 'accept() 漏存了这些凭据:' + chr(10) + joined
