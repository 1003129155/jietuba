...
- **任意来源自动入史**：不只是自己截的图，复制的任何文本/图片/文件，都会自动进入同一份剪切板历史

- **历史 → 钉图 → 历史**：历史里任何一张图都能一键"钉"回屏幕最前端，随时对照、缩放、继续标注

- **秒级检索**：关键词全文搜索 + 时间/类型筛选 + 自定义分组/收藏，成百上千条历史也能几秒定位

- **内存占用**：有严格的运行内存设计，后台常驻内存占用可以被压到非常低水平（往往仅在十几 MB 甚至几 MB 级别）

- **流畅 UI**：优化各种吃配置的场景，让低配电脑也可以流畅使用
